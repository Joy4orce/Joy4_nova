...
public class AnimesNShowsMapper implements CompatibleCursorMapper {
    ...
    public void bindColumns(Cursor c) {
        ...
        mIsShow = c.getColumnIndex("isShow");
        mUId = c.getColumnIndex("uId");
        mUName = c.getColumnIndex("uName");
        mUPoster = c.getColumnIndex("uPoster");
        ...
        mEpisodeCountColumn = c.getColumnIndex("episode_count");
        mEpisodeWatchedCountColumn = c.getColumnIndex("episode_watched_count");
    }
    ...
    public Object bind(Cursor c) {
        ...
        if (scraperType == BaseTags.TV_SHOW) {
            return new Tvshow(
                    ...
                    c.getString(mUName),
                    getPosterUri(c),
                    c.getInt(mSeasonCountColumn),
                    c.getInt(mEpisodeCountColumn),
                    mEpisodeWatchedCountColumn != -1 ? c.getInt(mEpisodeWatchedCountColumn) : -1,
                    VideoCursorMapper.isTraktSeenOrLibrary(c, mTraktSeenColumn),
    ...
    private Uri getPosterUri(Cursor c) {
        String path = c.getString(mUPoster);
        if (path!=null && !path.isEmpty()) {
            return Uri.parse("file://"+path);
        } else {
            return null;
