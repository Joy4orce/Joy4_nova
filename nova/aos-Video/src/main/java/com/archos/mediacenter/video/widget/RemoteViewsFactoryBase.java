...
abstract class RemoteViewsFactoryBase implements RemoteViewsService.RemoteViewsFactory {
    ...
    *****************************************************************/
    ...
    public void onCreate() {
        ...
        if (log.isDebugEnabled()) log.debug("onCreate : widget id={}", mAppWidgetId);
    ...
    public void onDestroy() {
        ...
        if (log.isDebugEnabled()) log.debug("onDestroy : widget id={}", mAppWidgetId);
        if (mCursor != null) {
    ...
    public int getCount() {
        if (log.isDebugEnabled()) log.debug("getDataSize() returns {}", mCursor.getCount());
        if (mCursor==null) {
            return 0;
        } else {
    ...
    public RemoteViews getViewAt(int position) {
        if (log.isDebugEnabled()) log.debug("getViewAt({})", position);
        ...
        if (log.isDebugEnabled()) log.debug("getViewAt {} for id={} => videoId={} name={}", position, mAppWidgetId, videoId, name);
    ...
    public void onDataSetChanged() {
        ...
        if (dataAvailable) {
            ...
            if (success) {
                ...
                if (log.isDebugEnabled()) log.debug("onDataSetChanged : {} items loaded for id={}", numItems, mAppWidgetId);

                if (numItems == 0) {
                    // No data
                    if (log.isDebugEnabled()) log.debug("No files found => set EMPTY_DATA_ACTION alarm at{}", currentElapsedRealTime);
                    ...
                    if (log.isDebugEnabled()) log.debug("Data now available => set UPDATE_ACTION alarm at{}", currentElapsedRealTime);
                    ...
                    if (log.isDebugEnabled()) log.debug("Data ok => set UPDATE_ACTION alarm at {}", currentElapsedRealTime);
                    setDelayedAlarm(WidgetProviderVideo.UPDATE_ACTION, UPDATE_ACTION_DELAY, true);
                }
            }
            else {
        ...
        if (!dataAvailable) {
            ...
            if (log.isDebugEnabled()) log.debug("Data not available yet => set RELOAD_ACTION alarm at {}", SystemClock.elapsedRealtime());
            setDelayedAlarm(WidgetProviderVideo.RELOAD_ACTION, RELOAD_ACTION_DELAY, false);
        }
    }

    public void registerDataSetObserver(DataSetObserver observer) {
        if (log.isDebugEnabled()) log.debug("registerDataSetObserver");
    }

    public void unregisterDataSetObserver(DataSetObserver observer) {
        if (log.isDebugEnabled()) log.debug("unregisterDataSetObserver");
    }
    ...
    *****************************************************************/
    ...
    public void onContentChanged() {
        ...
        long currentElapsedRealTime = SystemClock.elapsedRealtime();
        if (log.isDebugEnabled()) log.debug("onContentChanged : elapsed time since the last SHOW_UPDATE_SPINBAR_ACTION was sent = {}", (currentElapsedRealTime - mLastShowSpinbarTime));
        ...
        if (log.isDebugEnabled()) log.debug("onContentChanged : replace previous RELOAD_ACTION at {}", currentElapsedRealTime);
    ...
    *****************************************************************/
    ...
    private void setDelayedAlarm(String action, int delay, boolean notifyContentChanged) {
        if (log.isDebugEnabled()) log.debug("setDelayedAlarm: action {}, delay {}", action, delay);
    ...
    private void replacePreviousDelayedAlarm(String action, int delay) {
        ...
        if (log.isDebugEnabled()) log.debug("replacePreviousDelayedAlarm: action {}, delay {}", action, delay);
